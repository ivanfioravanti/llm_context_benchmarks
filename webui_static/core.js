...
(function () {
  ...
  const METRICS = [
    ...
    { key: "prompt_tps", label: "Prompt processing", unit: "tok/s",
      desc: "Prefill speed. Client-derived values remove the measured average endpoint latency; server-reported values are used unchanged." },
    { key: "prompt_tps_e2e", label: "Prompt processing (E2E)", unit: "tok/s",
      desc: "Unadjusted client prompt throughput: prompt tokens divided by end-to-end TTFT, including network and endpoint overhead." },
    { key: "endpoint_latency_ms", label: "Endpoint latency", unit: "ms",
      desc: "Average warm authenticated /models round-trip latency measured before the benchmark." },
    { key: "prompt_eval_duration", label: "Prompt time (adjusted)", unit: "s", seconds: true,
      desc: "Estimated prompt-processing duration after removing average endpoint latency, or server-reported prompt duration when available." },
