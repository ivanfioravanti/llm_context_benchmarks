#!/usr/bin/env python3
"""Benchmark script for oMLX using OpenAI-compatible API."""
...
OMLX_API_URL = "http://127.0.0.1:8000/v1"


def _make_cache_buster(run_idx: Optional[int] = None) -> str:
    """Generate a prefix to bust oMLX's KV cache.

    oMLX uses mlx-lm's continuous batching with tiered KV caching. When a new
    prompt shares a prefix with a previous request, cached tokens are reused,
    making prompt_eval_duration cover only the uncached delta while
    prompt_tokens reports the full length — inflating derived prompt t/s.
...
def ensure_endpoint(url: str) -> str:
    """Normalize an oMLX endpoint for OpenAI-compatible clients."""
...
def call_omlx(
    ...
    """Send a non-streaming chat completion request to oMLX."""
...
def run_benchmark(
    ...
    """Benchmark oMLX for a given context file."""
    print(f"Running benchmark for {context_file}...")

    with open(context_file, "r") as handle:
        prompt = handle.read()

    # Bust oMLX's KV cache by prepending a unique marker.
    ...
    try:
        ...
        print(f"Error contacting oMLX API: {exc}")
        return None
    ...
    # Prefer server-side metrics from oMLX when available (streaming with
    # include_usage returns prompt_eval_duration, generation_duration,
    # prompt_tokens_per_second, generation_tokens_per_second, etc.).
    server_prompt_tps = usage.get("prompt_tokens_per_second") or 0.0
    server_gen_tps = usage.get("generation_tokens_per_second") or 0.0
    server_prompt_eval = usage.get("prompt_eval_duration") or 0.0
    server_gen_duration = usage.get("generation_duration") or 0.0
    server_total_time = usage.get("total_time") or 0.0

    if server_prompt_eval > 0:
        prompt_eval_duration = server_prompt_eval
    if server_gen_duration > 0:
        generation_duration = server_gen_duration
    elif server_total_time > 0:
        generation_duration = max(server_total_time - prompt_eval_duration, 0.0)
    else:
    ...
    if prompt_eval_duration > 0:
        ...
        source = "server-side" if server_prompt_tps > 0 else "client-side"
        print(f"  Prompt throughput: {prompt_tps:.2f} tokens/sec ({source})")
        print(f"  Generation throughput: {generation_tps:.2f} tokens/sec ({source})")
...
def run_batch_benchmark(
    ...
    oMLX supports continuous batching via mlx-lm's BatchGenerator. This measures
    aggregate throughput under N concurrent clients — the server analog of
    mlx_lm.batch_generate.

    Args:
        base_url: oMLX server base URL (ending in /v1)
    ...
    def single_request() -> Dict:
        ...
        Prefers server-side timing (prompt_eval_duration, generation_duration)
        from oMLX's usage chunk when available; falls back to client-side TTFT.
        """
        import httpx
        ...
        # Use streaming with include_usage to get server-side metrics.
        ...
        # Prefer server-side timing from oMLX's usage extensions.
        server_ttft = usage.get("prompt_eval_duration") or usage.get("time_to_first_token") or 0.0
        server_gen_duration = usage.get("generation_duration") or 0.0
        ...
        if server_gen_duration > 0:
            decode_time = server_gen_duration
        else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run oMLX benchmarks using OpenAI-compatible API")
    parser.add_argument("model", help="Model id (e.g., gemma-4-26b-a4b-it-4bit)")

    common.setup_common_args(parser)

    parser.add_argument(
        "--api-key",
        help="API key (defaults to OMLX_API_KEY environment variable)",
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    parser.add_argument(
        "--base-url",
        default=OMLX_API_URL,
        help="Override oMLX API endpoint (default: http://127.0.0.1:8000/v1)",
    )
    parser.add_argument(
        "--request-model",
    ...
    parser.add_argument(
        ...
        default=True,
        help="Prepend a unique marker to every prompt to bust oMLX's KV "
    ...
    try:
        ...
        print(f"Error initializing oMLX client: {exc}")
        return 1
