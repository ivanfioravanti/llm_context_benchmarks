...
def run_cli_benchmark(model_name, context_file, max_tokens=32000):
...
def create_chart(results, model_name, hardware_info, output_path="benchmark_chart.png"):
    ...
    fig.suptitle(f"{model_name} Ollama CLI Testing\n{hardware_str}", fontsize=16, fontweight="bold")
...
def generate_table(results, model_name, hardware_info=None):
    ...
    # Create header
    table = f"{model_name} Ollama CLI Benchmark Results\n"
...
def main():
    ...
    output_dir = Path(f"benchmark_ollama_cli_{model_name}_{timestamp}")
